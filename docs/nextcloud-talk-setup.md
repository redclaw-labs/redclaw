...
This guide covers native Nextcloud Talk integration for RedClaw.
...
Add this section in `~/.redclaw/config.toml`:
...
When `webhook_secret` is configured, RedClaw verifies:
...
- RedClaw ignores bot-originated webhook events (`actorType = bots`).
- RedClaw ignores non-message/system events.
...
3. Confirm RedClaw receives and replies in the same room.
