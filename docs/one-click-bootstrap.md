...
bootstrap auto-falls back to `podman`. You can also set `REDCLAW_CONTAINER_CLI`
explicitly (for example: `REDCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
it pulls `ghcr.io/redclaw-labs/redclaw:latest` and tags it locally before running.
...
```bash
REDCLAW_API_KEY="sk-..." REDCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/redclaw-labs/redclaw:latest`)
- `--skip-install`
- `--provider <id>`
