# Thêm Board và Tool — Hướng dẫn phần cứng RedClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào RedClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.redclaw/config.toml)
redclaw peripheral add nucleo-f401re /dev/ttyACM0
redclaw peripheral add arduino-uno /dev/cu.usbmodem12345
redclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, RedClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `redclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `redclaw peripheral list` | Liệt kê các board đã cấu hình |
| `redclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `redclaw peripheral flash` | Nạp firmware Arduino |
| `redclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `redclaw hardware discover` | Liệt kê thiết bị USB |
| `redclaw hardware info` | Thông tin chip qua probe-rs |
