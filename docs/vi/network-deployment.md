# Triển khai mạng — RedClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai RedClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | RedClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | RedClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — RedClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. RedClaw trên Raspberry Pi
...
- Channel Telegram hoạt động: RedClaw poll Telegram API (outbound)
...
```bash
redclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `redclaw tunnel` (xem tài liệu tunnel).

3. RedClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- RedClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `redclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `redclaw daemon`).
...
- [ ] Chạy `redclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
