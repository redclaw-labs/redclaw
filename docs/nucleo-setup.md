...
|-----------|----------|---------|
| Firmware | `firmware/redclaw-nucleo/` | Embassy Rust — USART2 (115200), gpio_read, gpio_write |
...
This builds `firmware/redclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.

### 1.3 Manual Flash (Alternative)

```bash
cd firmware/redclaw-nucleo
cargo build --release --target thumbv7em-none-eabihf
probe-rs run --chip STM32F401RETx target/thumbv7em-none-eabihf/release/redclaw-nucleo
```
