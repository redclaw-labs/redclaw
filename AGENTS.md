# AGENTS.md — RedClaw Agent Engineering Protocol
...
RedClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use RedClaw-native labels only (`RedClawAgent`, `redclaw_user`, `redclaw_node`).
...
- If identity-like context is unavoidable, use RedClaw-scoped roles/labels only (for example: `RedClawAgent`, `RedClawOperator`, `redclaw_user`) and avoid real-world personas.
