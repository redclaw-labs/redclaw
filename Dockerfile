...
RUN cat > /redclaw-data/.redclaw/config.toml << 'EOF'
...
RUN chown -R 65534:65534 /redclaw-data
...
COPY --from=permissions /redclaw-data /redclaw-data
COPY --from=builder /app/target/release/redclaw /usr/local/bin/redclaw
