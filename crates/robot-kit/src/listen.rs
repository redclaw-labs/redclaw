...
impl ListenTool {
    pub fn new(config: RobotConfig) -> Self {
        let recordings_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".redclaw/recordings"))
            .unwrap_or_else(|| PathBuf::from("/tmp/redclaw_recordings"));
    ...
    async fn transcribe(&self, audio_path: &Path) -> Result<String> {
        ...
        // whisper.cpp model path (typically in ~/.redclaw/models/)
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".redclaw/models/ggml-{}.bin", model))
            })
            .unwrap_or_else(|| {
