...
async fn sqlite_memory_store_same_key_deduplicates() {
    ...
    mem.store("greeting", "hello world", MemoryCategory::Core)
        .await
        .unwrap();
    mem.store("greeting", "hello updated", MemoryCategory::Core)
        .await
        .unwrap();
...
async fn sqlite_memory_store_different_keys_creates_separate_entries() {
    ...
    mem.store("key_a", "content a", MemoryCategory::Core)
        .await
        .unwrap();
    mem.store("key_b", "content b", MemoryCategory::Core)
        .await
        .unwrap();
...
async fn sqlite_memory_persists_across_reinitialization() {
    ...
    {
        let mem = SqliteMemory::new(tmp.path()).unwrap();
        mem.store("persistent_fact", "Rust is great", MemoryCategory::Core)
            .await
            .unwrap();
    }
...
async fn sqlite_memory_restart_does_not_duplicate_on_rewrite() {
    ...
    {
        let mem = SqliteMemory::new(tmp.path()).unwrap();
        mem.store("fact_1", "original content", MemoryCategory::Core)
            .await
            .unwrap();
        mem.store("fact_2", "another fact", MemoryCategory::Core)
            .await
            .unwrap();
    }
...
async fn sqlite_memory_session_scoped_store_and_recall() {
    ...
    // Trait API no longer supports session-scoped listing; validate category listing.
    let session_a_entries = mem.list(Some(&MemoryCategory::Conversation)).await.unwrap();
    assert_eq!(
        session_a_entries.len(),
        2,
        "conversation category should include both entries"
    );
    let contents: Vec<&str> = session_a_entries
        .iter()
        .map(|e| e.content.as_str())
        .collect();
    assert!(contents.contains(&"fact from session A"));
    assert!(contents.contains(&"fact from session B"));
}
...
async fn sqlite_memory_global_recall_includes_all_sessions() {
    ...
    mem.store("global_a", "alpha content", MemoryCategory::Core)
        .await
        .unwrap();
    mem.store("global_b", "beta content", MemoryCategory::Core)
        .await
        .unwrap();
...
async fn sqlite_memory_recall_respects_limit() {
    ...
    let results = mem.recall("test content", 3).await.unwrap();
    assert!(
        results.len() <= 3,
...
async fn sqlite_memory_recall_empty_query_returns_empty() {
    ...
    mem.store("fact", "some content", MemoryCategory::Core)
        .await
        .unwrap();
...
async fn sqlite_memory_forget_removes_entry() {
    ...
    mem.store("to_forget", "temporary info", MemoryCategory::Core)
        .await
        .unwrap();
...
async fn sqlite_memory_list_by_category() {
    ...
    mem.store("core_fact", "core info", MemoryCategory::Core)
        .await
        .unwrap();
    mem.store("daily_note", "daily note", MemoryCategory::Daily)
        .await
        .unwrap();
    mem.store("conv_msg", "conversation msg", MemoryCategory::Conversation)
        .await
        .unwrap();
