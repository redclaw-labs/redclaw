...
usage() {
  cat <<'USAGE'
RedClaw installer bootstrap engine

Usage:
  ./redclaw_install.sh [options]
...
Examples:
  ./redclaw_install.sh
  ./redclaw_install.sh --guided
  ./redclaw_install.sh --install-system-deps --install-rust
  ./redclaw_install.sh --prefer-prebuilt
  ./redclaw_install.sh --prebuilt-only
  ./redclaw_install.sh --onboard --api-key "sk-..." --provider openrouter [--model "openrouter/auto"]
  ./redclaw_install.sh --interactive-onboard
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/redclaw-labs/redclaw/releases/latest/download/redclaw-${target}.tar.gz"
...
run_docker_bootstrap() {
  ...
  fallback_image="ghcr.io/redclaw-labs/redclaw:latest"
...
Use either:
  --api-key "sk-..."
or:
  REDCLAW_API_KEY="sk-..." ./redclaw_install.sh --docker
or run interactive:
  ./redclaw_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
...
REPO_URL="https://github.com/redclaw-labs/redclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
Next steps:
  ./redclaw_install.sh --docker --interactive-onboard
  ./redclaw_install.sh --docker --api-key "sk-..." --provider openrouter
DONE
  exit 0
fi
...
or re-run with:
  ./redclaw_install.sh --install-rust
MSG
  exit 1
fi
...
Use either:
  --api-key "sk-..."
or:
  REDCLAW_API_KEY="sk-..." ./redclaw_install.sh --onboard
or run interactive:
  ./redclaw_install.sh --interactive-onboard
MSG
      exit 1
