...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are RedClaw, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    ...
    if cfg!(target_os = "linux") {
        ...
        let active_output = Command::new("systemctl")
            .args(["--user", "is-active", "redclaw.service"])
            .output()
        ...
        let restart_output = Command::new("systemctl")
            .args(["--user", "restart", "redclaw.service"])
            .output()
