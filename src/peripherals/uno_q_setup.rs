...
const BRIDGE_APP_NAME: &str = "redclaw-uno-q-bridge";
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    let status = Command::new("ssh")
        .args([
            ...
            "app",
            "start",
            "~/ArduinoApps/redclaw-uno-q-bridge",
        ])
        .status()
...
fn write_embedded_bridge(dest: &std::path::Path) -> Result<()> {
    let app_yaml = include_str!("../../firmware/redclaw-uno-q-bridge/app.yaml");
    let sketch_ino = include_str!("../../firmware/redclaw-uno-q-bridge/sketch/sketch.ino");
    let sketch_yaml = include_str!("../../firmware/redclaw-uno-q-bridge/sketch/sketch.yaml");
    let main_py = include_str!("../../firmware/redclaw-uno-q-bridge/python/main.py");
    let requirements = include_str!("../../firmware/redclaw-uno-q-bridge/python/requirements.txt");
