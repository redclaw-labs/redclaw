...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["redclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "redclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["redclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "redclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["redclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["redclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/redclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/redclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "redclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove redclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/redclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// Check if the redclaw user exists and has expected properties.
...
fn check_redclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "redclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser redclaw && delgroup redclaw",
            "addgroup -S redclaw && adduser -S -s /sbin/nologin -H -D -G redclaw redclaw",
        )
    } else {
        ("userdel redclaw", "useradd -r -s /sbin/nologin redclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'redclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'redclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/redclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: redclaw user has home directory '{}' (expected /var/lib/redclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_redclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "redclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_redclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "redclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "redclaw"])
                .output()
                .context("Failed to create redclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create redclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: redclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create redclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create redclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "redclaw"])
            .output()
            .context("Failed to create redclaw user")?;
    ...
    println!("✅ Created system user: redclaw");
    Ok(())
}

/// Change ownership of a path to redclaw:redclaw
#[cfg(unix)]
fn chown_to_redclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["redclaw:redclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to redclaw:redclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_redclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_redclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "redclaw:redclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to redclaw:redclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_redclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'redclaw' cannot write {} ({details}). \
             Re-run `sudo redclaw service install` and ensure ownership is redclaw:redclaw.",
            path.display(),
        );
    }
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    ...
    let config_dir = Path::new("/etc/redclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/redclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_redclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by redclaw:redclaw)",
    ...
    chown_recursive_to_redclaw(config_dir)?;
    ...
    run_checked(Command::new("rc-update").args(["add", "redclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/redclaw");
    println!("   Config path: /etc/redclaw/config.toml");
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("redclaw-daemon.cmd");
...
mod tests {
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        assert!(script.contains("command_user=\"redclaw:redclaw\""));
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/redclaw"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/redclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/redclaw/workspace'".to_string(),
                "redclaw".to_string()
            ]
        );
    }
