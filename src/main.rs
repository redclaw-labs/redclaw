...
pub use redclaw::{HardwareCommands, PeripheralCommands};

/// `RedClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "redclaw")]
#[command(author = "RedClaw Labs")]
...
async fn main() -> Result<()> {
    ...
    } = &cli.command
    {
        ...
        let config = if *channels_only {
            ...
            onboard::run_wizard_with_force(*force).await?
        } else {
            onboard::run_quick_setup_with_force(
                api_key.as_deref(),
                provider.as_deref(),
                model.as_deref(),
                memory.as_deref(),
