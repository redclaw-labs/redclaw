...
impl Config {
    pub fn load_or_init() -> Result<Self> {
        ...
        if !redclaw_dir.exists() {
            fs::create_dir_all(&redclaw_dir).context("Failed to create .redclaw directory")?;
            fs::create_dir_all(redclaw_dir.join("workspace"))
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // Model: REDCLAW_MODEL
        if let Ok(model) = std::env::var("REDCLAW_MODEL") {
            if !model.is_empty() {
