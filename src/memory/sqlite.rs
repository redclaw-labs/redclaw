...
impl Memory for SqliteMemory {
    ...
    async fn recall(&self, query: &str, limit: usize) -> anyhow::Result<Vec<MemoryEntry>> {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<Vec<MemoryEntry>> {
            ...
            let vector_results = if let Some(ref qe) = query_embedding {
                Self::vector_search(&conn, qe, limit * 2, None, None).unwrap_or_default()
            } else {
                Vec::new()
            };
            ...
            if !merged.is_empty() {
                ...
                for row in rows {
                    let (id, key, content, cat, ts) = row?;
                    entry_map.insert(id, (key, content, cat, ts));
                }

                for scored in &merged {
                    if let Some((key, content, cat, ts)) = entry_map.remove(&scored.id) {
    ...
    async fn get(&self, key: &str) -> anyhow::Result<Option<MemoryEntry>> {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<Option<MemoryEntry>> {
            ...
            let mut rows = stmt.query_map(params![key], |row: &rusqlite::Row| {
                Ok(MemoryEntry {
                    id: row.get(0)?,
                    key: row.get(1)?,
                    content: row.get(2)?,
...
mod tests {
    ...
    async fn open_with_timeout_store_recall_unchanged() {
        ...
        .unwrap();
        mem.store("timeout_key", "value with timeout", MemoryCategory::Core)
            .await
            .unwrap();
    ...
    async fn sqlite_concurrent_read_write_no_panic() {
        ...
        // Pre-populate
        mem.store("shared_key", "initial", MemoryCategory::Core)
            .await
            .unwrap();
