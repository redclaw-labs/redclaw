...
fn create_cli_memory(config: &Config) -> Result<Box<dyn Memory>> {
    ...
    match classify_memory_backend(&backend) {
        ...
        MemoryBackendKind::Postgres => {
            ...
            {
                bail!(
                    "memory backend 'postgres' requires enabling the 'memory-postgres' cargo feature"
                );
            }
        }
...
async fn handle_list(
    ...
    let mut entries = mem.list(cat.as_ref()).await?;
...
async fn handle_stats(config: &Config) -> Result<()> {
    ...
    let all = mem.list(None).await.unwrap_or_default();
    if !all.is_empty() {
...
async fn handle_clear(
    ...
    let cat = category.as_deref().map(parse_category);
    let entries = mem.list(cat.as_ref()).await?;
