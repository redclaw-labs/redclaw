    ...
    async fn run_lucid_command_raw(
        ...
        let output = {
            ...
            match timeout(timeout_window, cmd.output()).await {
                ...
                Ok(Ok(output)) => output,
                Ok(Err(error)) => {
                    // Some platforms/runtimes can surface ENOEXEC for direct script execution.
                    // Fall back to running via bash so users can set REDCLAW_LUCID_CMD to a script wrapper.
                    if error.raw_os_error() == Some(8) {
                        let mut bash = Command::new("bash");
                        bash.arg(lucid_cmd);
                        bash.args(args);

                        timeout(timeout_window, bash.output()).await.map_err(|_| {
                            anyhow::anyhow!(
                                "lucid command timed out after {}ms",
                                timeout_window.as_millis()
                            )
...
"#,
        ...
        assert!(
            entries.iter().any(|e| e.content.contains("token refresh")),
            "Expected Lucid context merge; got:\n{entry_dump}"
        );
    }
...
"#
    ...
    async fn failure_cooldown_avoids_repeated_lucid_calls() {
        let tmp = TempDir::new().unwrap();
        let failing_cmd = write_failing_lucid_script(tmp.path());
        ...
        let first_failure_at = (*memory.last_failure_at.lock()).unwrap();
        let second = memory.recall("auth", 5).await.unwrap();
        let second_failure_at = (*memory.last_failure_at.lock()).unwrap();

        assert!(first.is_empty());
        assert!(second.is_empty());
        assert!(
            memory.in_failure_cooldown(),
            "Expected recall to enter failure cooldown after a lucid error"
        );
        assert_eq!(
            first_failure_at, second_failure_at,
            "Expected second recall to short-circuit due to cooldown (no repeated failure mark)"
        );
    }
}
