...
impl ResponseCache {
    ...
    pub fn stats(&self) -> Result<(usize, u64, u64)> {
        ...
        let count_usize = usize::try_from(count)
            .map_err(|_| anyhow::anyhow!("Invalid response cache count: {count}"))?;
        let hits_u64 = u64::try_from(hits)
            .map_err(|_| anyhow::anyhow!("Invalid response cache hits: {hits}"))?;
        let tokens_saved_u64 = u64::try_from(tokens_saved)
            .map_err(|_| anyhow::anyhow!("Invalid response cache tokens_saved: {tokens_saved}"))?;

        Ok((count_usize, hits_u64, tokens_saved_u64))
    }
