...
// in `~/.redclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.redclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(redclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: redclaw_dir.join(".secret_key"),
            enabled,
        }
    }
