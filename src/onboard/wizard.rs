use crate::config::schema::{DingTalkConfig, IrcConfig, LinqConfig, QQConfig, WhatsAppConfig};
use crate::config::{
...
fn has_launchable_channels(channels: &ChannelsConfig) -> bool {
    let ChannelsConfig {
        ...
        webhook: _, // webhook traffic is handled by gateway, not `redclaw channel start`
        telegram,
        discord,
        slack,
        mattermost,
...
pub async fn run_wizard_with_force(force: bool) -> Result<Config> {
    ...
    let config = Config {
        ...
        cron: crate::config::schema::CronConfig::default(),
        channels_config,
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".redclaw");
    ...
    let redclaw_dir = if use_default {
        default_dir
    } else {
        ...
        match choice {
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— Lark/Feishu bot").dim()
                );
                print_bullet("1. Create a Lark/Feishu app and enable bot");
                print_bullet("2. Copy App ID and App Secret");
                print_bullet("3. Paste below");
                println!();
