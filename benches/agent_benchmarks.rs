...
use redclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use redclaw::config::MemoryConfig;
use redclaw::memory;
use redclaw::memory::{Memory, MemoryCategory};
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| rt.block_on(async { mem.recall(black_box("redclaw agent"), 10).await.unwrap() }));
    });
