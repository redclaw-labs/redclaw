#!/bin/bash
# RedClaw Telegram Integration Test Suite
...
if [ -f "target/release/redclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/redclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/redclaw 2>/dev/null || stat -c%s target/release/redclaw)
...
CONFIG_PATH="$HOME/.redclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'redclaw onboard' first"
    fi
else
    warn "No config file found - run 'redclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/redclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
