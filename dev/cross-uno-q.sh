#!/usr/bin/env bash
# Cross-compile RedClaw for Arduino UNO Q (aarch64 Debian Linux).
#
# Prerequisites:
...
echo "==> Cross-compiling RedClaw for $TARGET ($PROFILE)"
...
BINARY="$PROJECT_DIR/target/$TARGET/$( [[ $PROFILE == release ]] && echo release || echo debug )/redclaw"
...
if [[ -f "$BINARY" ]]; then
    ...
    echo "  redclaw peripheral deploy-uno-q --host <uno-q-ip>"
    echo ""
    echo "Or manually:"
    echo "  scp $BINARY arduino@<uno-q-ip>:~/redclaw/"
else
